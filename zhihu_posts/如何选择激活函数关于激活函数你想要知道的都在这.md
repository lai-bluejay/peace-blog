...
<br>
<br><h3>激活函数</h3><br>
<br>对于神经网络，网络的每一层计算可以理解为$f(wx+b)=f(w\'x)$，每一层可以认为是每个权重矩阵$W$ 乘输入特征矩阵$X$。根据矩阵乘法，如果是线性激励，即激活函数为$f(x) = x$，多层网络相当于一层网络。比如：$​​f(W_1*f(W_2x))=W_1 W_2x=Wx$ 。
<br>
<br>因此，需要引入非线性激活函数，对特征进行充分组合。也可以说相当于进行空间变换，使得原来线性不可解的问题有非线性的表达，找到非线性的解。
<br>
<br><h3>好的激活函数的特性</h3><br>
<br>1. 非线性，保证不会退回线性网络。
<br>
...
<br>
<br><h4>如何选择激活函数</h4><br>
<br>1. 首先尝试ReLU,速度快,但要注意训练的状态.
...
<br>
<br><h4>特例：LSTM</h4><br>
<br>sigmoid/tanh在RNN（LSTM、注意力机制等）结构中有所应用，作为门控或者概率值。对于用在了状态和输出上，是对数据的处理，使用何种激活函数参考上述内容。
<br>
